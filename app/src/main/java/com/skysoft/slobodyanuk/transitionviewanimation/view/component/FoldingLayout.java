...
public class FoldingLayout extends ViewGroup {
    ...
    public void setFoldListener(OnFoldListener foldListener, FoldItem item, RecyclerView.ViewHolder holder) {
    ...
    */
    private void calculateMatrices() {
        ...
        if (mFoldFactor == 1 && mPreviousFoldFactor > 0) {
            mShouldDraw = false;
            mFoldListener.onEndFold(mFoldItem, holder);
        }

        if (mPreviousFoldFactor == 0 && mFoldFactor > 0) {
            mFoldListener.onStartFold(mFoldItem, holder);
        }
